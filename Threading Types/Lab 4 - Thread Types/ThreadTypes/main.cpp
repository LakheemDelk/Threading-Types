...
////////////////////////////////////////////////////////////////////////////////////
void Print(ThreadStruct* threadData, std::promise<int>* futureWork)
{
	printf("\tThread id: %d, Work Done = %d\n", threadData->id, futureWork->get_future().get());
}
///////////////////////////////////////////////////////////////////////////////////
...
void JoinableThreadEntrypoint(ThreadStruct *threadData)
{
	...
	int work = 0;
	std::promise<int> proWork;
	std::thread printerThread = std::thread(Print, threadData, &proWork);
...
void DetachedThreadEntrypoint(ThreadStruct *threadData)
{
	...
	{ 
		...
		///////////////////////////////////////////////////////////////////////////////////
		std::unique_lock<std::mutex> leave(mTex);
		if (threadData -> death)
		{
			break;
		}
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> detachCount(mTex);
	detached--;
	cond.notify_all();
	detachCount.unlock();
}
...
int main(int argc, char **argv)
{
	...
	std::vector<std::thread> joinedThreads;
	...
	{		
		...
		if (i % 2 == 0)
		{
			joinedThreads.push_back(std::thread(JoinableThreadEntrypoint, &perThreadData[i]));
		}
		else
		{
			std::unique_lock<std::mutex> mainLock(mTex);
			detached++;
			mainLock.unlock();
			detachedThreads.push_back(std::thread(DetachedThreadEntrypoint, &perThreadData[i]));
			detachedThreads[detachedThreads.size() - 1].detach();
		}
	}
	...
	///////////////////////////////////////////////////////////////////////////////////	
	//Would give an error for whatever reason so I made an int to fix that problem
	int error = joinedThreads.size();
	for (int i = 0; i < error; i++)
	{
		joinedThreads[i].join();
	}
	///////////////////////////////////////////////////////////////////////////////////
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> finalLock(mTex);
	cond.wait(finalLock, [] {return detached == 0; });
	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Cleanup
	///////////////////////////////////////////////////////////////////////////////////
